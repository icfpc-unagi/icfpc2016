...
public class Utils {
	...
	/**
	 * [from, to)の範囲でis[i]>=vとなる最小のiを求める．
	 */
	public static int lowerBound(int[] is, int from, int to, int v) {
		int s = from - 1, t = to;
		while (t - s > 1) {
			int m = (s + t) >>> 1;
			if (is[m] >= v) t = m;
	 ...
	 * [from, to)の範囲でis[i]>vとなる最小のiを求める．
	 */
	public static int upperBound(int[] is, int from, int to, int v) {
		int s = from - 1, t = to;
		while (t - s > 1) {
			int m = (s + t) >>> 1;
			if (is[m] > v) t = m;
	 ...
	 * 挿入された場所を返す．
	 */
	public static int insert(int[] is, int length, int v) {
		int p = upperBound(is, 0, length, v);
		System.arraycopy(is, p, is, p + 1, length - p);
		is[p] = v;
		return p;
	}
	 ...
	 * 配列の辞書順Comparator
	 */
	public static class CompArray implements Comparator<int[]> {
		@Override
	...
	/**
	 * 配列を指定した区切り文字を挟んで文字列に変換
	 */
	public static String toString(int[] a, String s) {
	 ...
	 * 配列長を伸ばす際の倍数．
	 */
	public static double GROW_FACTOR = 1.5;
	
	public static int[] grow(int[] is) {
		int newLength = (int)min(Integer.MAX_VALUE - 8, (long)(is.length * GROW_FACTOR + 1));
		return copyOf(is, newLength);
	}
